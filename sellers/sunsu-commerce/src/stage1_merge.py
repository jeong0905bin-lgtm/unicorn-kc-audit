...
def main():
    ...
    verified=sorted(store.values(),key=lambda x:int(x['productId']))
    candidates=[]
    for row in verified:
        raw=json.dumps(row.get('rawListing') or {},ensure_ascii=False)
        text=' '.join([row.get('productName',''),raw])
        if UNICORN_RE.search(text):
            row['publisherGrade']='확정';row['publisherReason']='판매자 목록 API 상품명/메타데이터 직접 표기'
            candidates.append(row)
        elif any(k in row.get('productName','') for k in ['퍼즐','스티커','색칠북','컬렉션북','워터','놀이북']):
            row['publisherGrade']='후보';row['publisherReason']='ISBN·공식 도서정보 교차검증 필요'
            candidates.append(row)
        else:
            row['publisherGrade']='미확정';row['publisherReason']='직접 근거 없음'
    summary={'discoveredUniqueProductIds':len(store),'sellerConfirmed':len(verified),'remainingUnresolved':0,'expectedCount':EXPECTED,'catalogComplete':len(verified)==EXPECTED,'publisherCandidates':len(candidates),'invalidShardFiles':len(invalid)}
    payload={'seller':{'name':SELLER,'sellerId':SELLER_ID,'storeId':STORE_ID,'expectedCount':EXPECTED},'summary':summary,'shardCounts':shard_counts,'products':verified,'unresolvedProducts':[],'invalidInputs':invalid,'generatedAt':now()}
    ...
    a.summary_out.write_text(json.dumps(summary,ensure_ascii=False,indent=2),encoding='utf-8')
    print(summary,flush=True)
    if len(verified)!=EXPECTED:raise SystemExit(f'stage1 incomplete: expected {EXPECTED}, listing-api products {len(verified)}')
