...
def request_query(session, query, store, events, sorts=SORTS):
    for sort in sorts:
        ...
        try:
            ...
            products=(body.get('data') or {}).get('products') or []; added=0
            for raw in products:
            ...
            if added: print({'query':query or 'ALL','sort':sort,'added':added,'total':len(store)},flush=True)
        except Exception as e: events.append({'query':query,'sort':sort,'error':f'{type(e).__name__}: {e}'})
        time.sleep(.02)

def derived_queries(store:dict[str,dict[str,Any]])->list[str]:
    q=set()
    for row in store.values():
        name=re.sub(r'\s+','',row.get('productName',''))
        name=re.sub(r'[^0-9A-Za-z가-힣]','',name)
        for n in (2,3,4):
            for i in range(max(0,len(name)-n+1)):
                token=name[i:i+n]
                if any('가'<=c<='힣' for c in token): q.add(token)
    return sorted(q)
...
def main():
    ...
    for query in selected: request_query(session,query,store,events)
    adaptive=[q for i,q in enumerate(derived_queries(store)) if i%a.shards==a.shard]
    # Adaptive title fragments target hidden catalog items sharing uncommon series/model words.
    for query in adaptive[:1200]: request_query(session,query,store,events,sorts=['POPULARITY','LATEST'])
    rows=sorted(store.values(),key=lambda x:int(x['productId']))
    out={'seller':SELLER,'sellerId':SELLER_ID,'storeId':STORE_ID,'shard':a.shard,'shards':a.shards,'queryCount':len(selected),'adaptiveQueryCount':min(len(adaptive),1200),'count':len(rows),'events':events,'products':rows,'generatedAt':now()}
    a.out.parent.mkdir(parents=True,exist_ok=True); a.out.write_text(json.dumps(out,ensure_ascii=False,indent=2),encoding='utf-8')
    print({'shard':a.shard,'finalCount':len(rows),'adaptive':min(len(adaptive),1200)},flush=True)
